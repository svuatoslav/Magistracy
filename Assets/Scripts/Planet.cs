    ...
    private void Awake()
    {
        ...
        if (Orbit == Orbits.Elliptical)
        {
            ClassOrbit = new Elliptic(Eccentricity, Satellite.q  / (1 - Eccentricity), Satellite.Alpha, Satellite.r0,
                Satellite.Omega0, (new EulerAngles(Satellite.Phi0, Satellite.Psi0, Satellite.Theta0), new DimensionlessPulses(Satellite.Pphi0, Satellite.Ppsi0, Satellite.Ptheta0)), Run.Instance.regularPrecession);
            transform.position = new Vector3((float)(StartPosition / Run.Instance.Scale), 0, 0);
        }
        else
        {

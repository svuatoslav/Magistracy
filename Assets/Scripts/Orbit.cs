...
abstract public class Orbit
{
    ...
    private protected readonly (EulerAngles, DimensionlessPulses) StartAngle;
    internal List<Func<double, (EulerAngles, DimensionlessPulses), double>> ODEMotions { get; private protected set; }
    public Orbit(double alpha, double beta, double r0, double omega0, (EulerAngles, DimensionlessPulses) startAngle)
    {
        Alpha = alpha;
        Beta = beta;
        Omega0 = omega0;
...
public class Elliptic : Orbit
{
    ...
    public Elliptic(double e, double a, double alpha, double beta, double r0, double omega0, (EulerAngles, DimensionlessPulses) startParam, RegularPrecessions regularPrecession) : base(alpha, beta, r0, omega0, startParam)
    {
        this.e = e;
        this.a = a;
        b = a * Sqrt(1 - Pow(e, 2));
...
public class Circular : Orbit
{
    ...
    public Circular(double R, double alpha, double beta, double r0, double omega0, (EulerAngles, DimensionlessPulses) startParam) : base(alpha, beta, r0, omega0, startParam) 
    { 
        this.R = R;
        ODEMotions = new() { Diff_phi, Diff_psi, Diff_theta, Diff_pphi, Diff_ppsi, Diff_ptheta };
    }
    ...
    private protected override double Diff_phi(double nu, (EulerAngles, DimensionlessPulses) motions) => Omega0 * (Beta + Cos(StartAngle.Item1.psi) * Sin(StartAngle.Item1.theta));
